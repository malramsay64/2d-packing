...
void define_fourier(struct shapetype *shape, double fcoeff[2][FOURIER_TERMS]) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    for (j = 0; j < FOURIER_TERMS; j++) {
      shape->r[i] +=
          fcoeff[0][j] * cos((j + 1) * phi) + fcoeff[1][j] * sin((j + 1) * phi);
    }
    if (shape->r[i] <= 0) {
      fprintf(stderr, "shape has 0 or negative radius at phi=%f\n", phi);
      exit(1);
    }
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
  }

  // find rotsym. if all coeffs are zero, rotsym is number of points. if any 1
  // coeffs are nonzero, rotsym is 1. otherwise, proceed through all other
  // coeffs, find hcf.
  if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) && (fcoeff[0][2] == 0) &&
      (fcoeff[0][3] == 0) && (fcoeff[0][4] == 0) && (fcoeff[0][5] == 0) &&
      (fcoeff[0][6] == 0) && (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0) &&
      (fcoeff[1][0] == 0) && (fcoeff[1][1] == 0) && (fcoeff[1][2] == 0) &&
      (fcoeff[1][3] == 0) && (fcoeff[1][4] == 0) && (fcoeff[1][5] == 0) &&
      (fcoeff[1][6] == 0) && (fcoeff[1][7] == 0) && (fcoeff[1][8] == 0)) {
  ...
  // if there are any sine terms, then there is not a mirror on the x-axis.
  // otherwise, there is. BUT on 2013-04-02 - we realised that although this
  // statement is strictly true, (i) shapes with sine terms can have mirrors
  // elsewhere which we must consider, and (ii) shapes with cos terms can have
  // more than one mirror. So we temporarily patched this part of code up, so now
  // it works (a) for non-zero coefficients only from 2 to 6, and (b) when you
  // input the coefficients as all cos terms if there was originally a sine term
  // and the shape had one or more mirrors. (i.e. input the same shape using
  // different coefficient values, by expressing shape with only cos's,
  // effectively rotating the shape so it's mirror is on the x axis). mirrors
  // after the first mirror on the x axis must be in certain positions (e.g.
  // perpendicular if there are 2 mirrors). so first: if any (non-zero) sine
  // terms, no mirrors.
  if ((fcoeff[1][0] != 0) || (fcoeff[1][1] != 0) || (fcoeff[1][2] != 0) ||
      (fcoeff[1][3] != 0) || (fcoeff[1][4] != 0) || (fcoeff[1][5] != 0) ||
      (fcoeff[1][6] != 0) || (fcoeff[1][7] != 0) || (fcoeff[1][8] != 0)) {
    shape->mirrors = 0;
  } else {
    // so now no sine terms. next, if shape is a single (i.e. only one term, a
    // cos), then same number of mirrors as order of cos term. NB i have ignored
    // the possibility of the single cos term being first order, since i never
    // use these.
    if ((fcoeff[0][0] == 0) && (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
        (fcoeff[0][4] == 0) && (fcoeff[0][5] == 0) && (fcoeff[0][6] == 0) &&
        (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 2;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][3] == 0) && (fcoeff[0][4] == 0) &&
               (fcoeff[0][5] == 0) && (fcoeff[0][6] == 0) &&
               (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 3;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][4] == 0) &&
               (fcoeff[0][5] == 0) && (fcoeff[0][6] == 0) &&
               (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 4;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
               (fcoeff[0][5] == 0) && (fcoeff[0][6] == 0) &&
               (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 5;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
               (fcoeff[0][4] == 0) && (fcoeff[0][6] == 0) &&
               (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 6;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
               (fcoeff[0][4] == 0) && (fcoeff[0][5] == 0) &&
               (fcoeff[0][7] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 7;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
               (fcoeff[0][4] == 0) && (fcoeff[0][5] == 0) &&
               (fcoeff[0][6] == 0) && (fcoeff[0][8] == 0)) {
      shape->mirrors = 8;
    } else if ((fcoeff[0][0] == 0) && (fcoeff[0][1] == 0) &&
               (fcoeff[0][2] == 0) && (fcoeff[0][3] == 0) &&
               (fcoeff[0][4] == 0) && (fcoeff[0][5] == 0) &&
               (fcoeff[0][6] == 0) && (fcoeff[0][7] == 0)) {
      shape->mirrors = 9;
    } else if ((fcoeff[0][0] != 0) || (fcoeff[0][4] != 0) ||
               (fcoeff[0][6] != 0) || (fcoeff[0][8] != 0)) {
      // so now no sine terms and not a single. next, if any (non-zero) cos
      // order 1, 5, 7 or 9 (i.e. all odds except 3), there is one mirror, on the
      // x axis.
      shape->mirrors = 1;
    } else if ((fcoeff[0][2] != 0)) {
      // so now, no sine terms and not a single and no cos terms of order 1, 5,
      // 7, or 9. next, if cos order 3 is non-zero, there are three mirrors if
      // only other term is cos 6, otherwise, 1 mirror.
      if ((fcoeff[0][5] != 0) && (fcoeff[0][1] == 0) && (fcoeff[0][3] == 0) &&
          (fcoeff[0][7] == 0)) {
        shape->mirrors = 3;
      } else {
        shape->mirrors = 1;
      }
    } else if ((fcoeff[0][0] == 0) || (fcoeff[0][2] == 0) ||
               (fcoeff[0][4] == 0) || (fcoeff[0][6] == 0) ||
               (fcoeff[0][8] == 0)) {
      // so now, no sine terms and not a single and no cos terms of odd order.
      // then there are 2 mirrors.
      shape->mirrors = 2;
    }
  }
}
...
/* DAN shape_var = 1 gives a circle, shape_var = 0 gives a straight line.
 * eccentricity is sqrt((0.25-pow((0.5*shape_var),2)/0.25) */
void define_ellipse(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    shape->r[i] =
        (0.5 * (0.5 * shape_var)) /
        sqrt(pow((0.5 * cos(phi)), 2) + pow(((0.5 * shape_var) * sin(phi)), 2));
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
...
void define_wobblyfan3(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    phi = 2.0 * M_PI * i / SHAPE_RESOLUTION;
    shape->r[i] = 0.2 + 0.6 * fabs(sin(3 * phi / 2.0)) *
                            fmod(3.0 * phi / (2.0 * M_PI), 1.0);
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
...
void define_pentagon(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    while (phi > 2.0 * M_PI / 5.0)
      phi -= 2.0 * M_PI / 5.0;
    shape->r[i] = sin(0.5 * M_PI * (5 - 2) / 5.0) /
                  sin(M_PI - 0.5 * M_PI * (5 - 2) / 5.0 - phi);
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
...
void define_pentagoncircle(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    while (phi >= beta)
      phi -= beta;
    theta_dash =
        M_PI - phi - alpha - asin(d_centres * sin(phi + alpha) / shape_var);
    shape->r[i] = sqrt(pow(shape_var, 2) + pow(d_centres, 2) -
                       2 * shape_var * d_centres * cos(theta_dash));
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
...
void define_RichardJames_pentagon(struct shapetype *shape, double angle_D) {
  ...
  side_d = (1.0 + cos(angle_D) + (tan(angle_D / 2.0)) * (sin(angle_D) - 1)) /
           (sin(angle_D) + cos(angle_D) +
            tan(angle_D / 2.0) * (sin(angle_D) - cos(angle_D)));
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    if ((phi > 5.0 * M_PI / 4.0) && (phi < 7.0 * M_PI / 4.0)) {
    ...
    } else if (((phi > 7.0 * M_PI / 4.0) && (phi < 2.0 * M_PI + theta_1)) ||
               (phi < theta_1)) {
      // this is segment AB
      x1 = 1.0;
      incline = angle_D;
      x_int = (x1 * tan(incline) + 0.5 - 0.5 * tan(phi)) /
              (tan(incline) - tan(phi));
      ...
      // this is segment BC
      incline = (angle_D - M_PI) / 2.0;
      x1 = side_d * cos(angle_D - (M_PI / 2.0)) -
           ((1.0 + side_d * sin(angle_D - (M_PI / 2.0))) /
            tan((angle_D - M_PI) / 2.0));
      x_int = (x1 * tan(incline) + 0.5 - 0.5 * tan(phi)) /
              (tan(incline) - tan(phi));
      y_int = (x_int - x1) * tan(incline);
      shape->r[i] = sqrt(pow(x_int - 0.5, 2.0) + pow(y_int - 0.5, 2.0));
...
void define_MarjorieRice_pentagon(struct shapetype *shape, double angle_B) {
  ...
  side_b = 2.0 * side_c * sin(angle_B) +
           sqrt(2.0) * side_c * cos(angle_B - M_PI / 4.0);
  ...
  printf("MR pentagon theta_b %f theta_c %f theta_d %f side_b %f side_c %f xc "
         "%f yc %f xd %f yd %f\n",
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    shape->r[i] *=
        0.25; /* to make it a similar size to the unit radius shapes */
    if (shape->r[i] > shape->maxr)
      shape->maxr = shape->r[i];
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
...
void define_bowtie(struct shapetype *bowtie) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    } else if (bowtie->r[i] < bowtie->minr)
      bowtie->minr = bowtie->r[i];
  }
...
void define_triangle(struct shapetype *triangle) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    while (
        phi >
        M_PI) { /* && phi != 2*M_PI/3 && phi != 5*M_PI/3 && phi != 7*M_PI/6 */
      phi = 2 * M_PI - phi;
    }
    ...
    if (triangle->r[i] < triangle->minr)
      triangle->minr = triangle->r[i];
  }
  triangle->rotsym = rot_symmetry;
  triangle->mirrors = 3;
}

void define_smoothedpentagon(
    struct shapetype *shape) { /* NOT WORKING- needs to be fixed*/
  int i;
  double phi;
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    if (phi >= 0 * M_PI && phi <= 0 * M_PI + angle1) {
      ...
      double a =
          A * pow(cos(2 * M_PI / 5 - phi), 2) - pow(sin(2 * M_PI / 5 - phi), 2);
...
void define_hexagon (struct shapetype *shape) {
  ...
  for (i=0; i<SHAPE_RESOLUTION; i++) {
    ...
    // to implement the rotational symmetry
    while (phi>2.0*M_PI/rot_symmetry && phi!=2.0*M_PI/3 && phi!=M_PI) {   //is
that correct? phi -= 2.0*M_PI/rot_symmetry;
    };
...
void define_smoothedoctagon(struct shapetype *shape) {
  ...
  double l = sqrt(2) - 1;
  // double l2 = pow(l,2);
  double d1 = sqrt(pow(((2 * sqrt(2) + 2) / (sqrt(2) + 1 - l)) - 2, 2) +
                   pow((2 * l * (sqrt(2) + 1)) / (sqrt(2) + 1 - l), 2));
  // double d1 = 1.0823922;
  double d2 = (d1 * sin(3 * M_PI / 8)) / sin(M_PI / 4);
  // double d2 = 1.414213562;
  double n = ((-4) * pow(tan(3 * M_PI / 8), 2) + sqrt(4.7 * pow(10, -10))) /
             (6 - 4 * sqrt(2) - 2 * pow(tan(3 * M_PI / 8), 2));
...
void define_fan3(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    if ((phi >= 0 * M_PI) && (phi < M_PI / 3)) {
      shape->r[i] =
          0.7 + (fabs((sin(M_PI / 6)) / (sin(5 * M_PI / 6 - phi / 6))));
      shape->r[i] *= shape->r[i];
...
void define_flower5(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    if (phi >= 0 * M_PI && phi < 2 * M_PI / 15) {
      ...
      shape->r[i] =
          0.7 + (fabs((sin(M_PI / 15)) / (sin(14 * M_PI / 15 - phi / 15))));
...
void define_fan12(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    ...
    // specifying all the values of the radius
    if (phi >= 0 * M_PI && phi <= M_PI / 6) {
      shape->r[i] = ((sqrt(3) / 2) * sin(M_PI / 24) / (sin(M_PI / 2 - phi))) +
                    (0.5 * cos(3 * phi));
    }
    if (shape->r[i] < shape->minr)
      shape->minr = shape->r[i];
