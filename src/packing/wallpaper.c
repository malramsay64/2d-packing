...
int enumerate(int OS[][MAXNUMOCCSITES], int maxcombinations, int n_comb, int os,
              int bag_i, int combination_bag[], int bag_size) {
  int i, j;
  int count = 0, new;
  ...
  if (os == numoccsites)
    ...
    for (i = bag_i; i < bag_size - (numoccsites - os - 1); i++) {
      if ((i != bag_i) && (combination_bag[i] == combination_bag[i - 1]))
        continue;
      new = enumerate(OS, maxcombinations, n_comb + count, os + 1, i + 1,
                      combination_bag, bag_size);
      for (j = 0; j < new; j++) {
...
double area(struct shapetype *shape) {
  ...
  for (i = 0; i < SHAPE_RESOLUTION; i++) {
    // old   areasum +=
    // 0.5*pow(0.5*(shape->r[i]+shape->r[(i+1)%SHAPE_RESOLUTION]),2.0)*(2.0*M_PI/SHAPE_RESOLUTION);
    // the following is the exact triangle area
    areasum += 0.5 * (shape->r[i]) * (shape->r[(i + 1) % SHAPE_RESOLUTION]) *
               sin(2.0 * M_PI / SHAPE_RESOLUTION);
  }
  // printf("area %f\n", areasum);
  return areasum;
}

void fractcoords(double coord_coeffs[DIM][DIM + 1], double *sitevariables[3],
                 double newfcoords[DIM]) {
  /* converts site variables and wyckoff site coefficients into the location of
   * the actual wyckoff image in fractional coordinates */
  newfcoords[0] = coord_coeffs[0][0] * (*sitevariables[0]) +
                  coord_coeffs[0][1] * (*sitevariables[1]) + coord_coeffs[0][2];
  while (newfcoords[0] < 0.0)
    newfcoords[0] += 1.0;
  while (newfcoords[0] > 1.0)
    newfcoords[0] -= 1.0;
  newfcoords[1] = coord_coeffs[1][0] * (*sitevariables[0]) +
                  coord_coeffs[1][1] * (*sitevariables[1]) + coord_coeffs[1][2];
  while (newfcoords[1] < 0.0)
    newfcoords[1] += 1.0;
  while (newfcoords[1] > 1.0)
    newfcoords[1] -= 1.0;
  return;
}

void realcoords(double fcoords[DIM], double coords[DIM], double *cellsides[DIM],
                double *cellangles[DIM - 1]) {
  // printf("cell %f %f %f\n", (*cellsides[0]), (*cellsides[1]),
  // (*cellangles[0]));
  coords[0] = fcoords[0] * (*cellsides[0]) +
              fcoords[1] * (*cellsides[1]) * cos(*cellangles[0]);
  coords[1] = fcoords[1] * (*cellsides[1]) * sin(*cellangles[0]);
  return;
}

bool segments_cross(double Ax, double Ay, double Bx, double By, double Cx,
                    double Cy, double Dx, double Dy) {
  //  Determines the intersection point of the line segment defined by points A
  //  and B with the line segment defined by points C and D.
  //
  //  Returns YES if the intersection point was found, and stores that point in
  //  X,Y. Returns NO if there is no determinable intersection point, in which
...
void dump_clash(struct shapetype *shape_a, struct shapetype *shape_b,
                double coords_a[DIM], double coords_b[DIM], double angle_a,
                double angle_b, bool flip_a, bool flip_b,
  ...
  for (scan_sign = 1; scan_sign >= -1; scan_sign -= 2) {
    for (ib = -1; ib < (SHAPE_RESOLUTION / 4) + 1; ib++) {
      theta_tab[ib + 1] =
          fabs((angle_b_int - (1 - 2 * flip_b) * scan_sign * ib) * 2.0 * M_PI /
                   SHAPE_RESOLUTION -
               angle_b);
      // sine_theta_ib_tab[ib] = sin(theta_tab[ib]);
      xb_tab[ib + 1] =
          shape_b->r[(angle_b_int - (1 - 2 * flip_b) * scan_sign * ib +
                      5 * SHAPE_RESOLUTION) %
                     SHAPE_RESOLUTION] *
          cos(theta_tab[ib + 1]);
      yb_tab[ib + 1] =
          ...
          sin(theta_tab[ib + 1]);
      if (ib == -1)
        yb_tab[ib + 1] *= -1.0; /* to correct an issue with the fabs of the very
                                   first point */
    ...
    theta_ia1 = fabs((angle_a_int + (1 - 2 * flip_a) * scan_sign * -1) * 2.0 *
                         M_PI / SHAPE_RESOLUTION -
                     angle_a);
    xa1 = sqrt(central_distsq) -
          shape_a->r[(angle_a_int + (1 - 2 * flip_a) * scan_sign * -1 +
                      5 * SHAPE_RESOLUTION) %
                     SHAPE_RESOLUTION] *
              cos(theta_ia0);
    ya1 = shape_a->r[(angle_a_int + (1 - 2 * flip_a) * scan_sign * -1 +
                      5 * SHAPE_RESOLUTION) %
                     SHAPE_RESOLUTION] *
          sin(theta_ia0);
    ...
    for (ia = -1; ia < (SHAPE_RESOLUTION / 4) + 1; ia++) {
      ...
      theta_ia0 = theta_ia1;
      theta_ia1 = fabs((angle_a_int + (1 - 2 * flip_a) * scan_sign * (ia + 1)) *
                           2.0 * M_PI / SHAPE_RESOLUTION -
                       angle_a);
      xa0 = xa1;
      ya0 = ya1;
      xa1 = centraldist -
            shape_a->r[(angle_a_int + (1 - 2 * flip_a) * scan_sign * (ia + 1) +
                        5 * SHAPE_RESOLUTION) %
                       SHAPE_RESOLUTION] *
                cos(theta_ia1);
      ya1 = shape_a->r[(angle_a_int + (1 - 2 * flip_a) * scan_sign * (ia + 1) +
                        5 * SHAPE_RESOLUTION) %
                       SHAPE_RESOLUTION] *
            sin(theta_ia1);
...
bool indirect_clash(struct shapetype *shape_a, struct shapetype *shape_b,
                    double coords_a[DIM], double coords_b[DIM], double angle_a,
                    double angle_b, bool flip_a, bool flip_b,
                    double central_distsq, int tests) {
  ...
  for (scan_sign = 1; scan_sign >= -1; scan_sign -= 2) {
    ...
    if (tests == 0) {
      // If we have just initialised (before doing any moves), and are checking
      // for clashes, we want to check all the way around the edge of each shape,
      // to make sure that by chance the two shapes weren't positioned on top of
      // each other.
      for (ib = -1; ib < SHAPE_RESOLUTION / 2; ib++) {
        theta_tab[ib + 1] =
            fabs((angle_b_int - (1 - 2 * flip_b) * scan_sign * ib) * 2.0 *
                     M_PI / SHAPE_RESOLUTION -
                 angle_b);
        // sine_theta_ib_tab[ib] = sin(theta_tab[ib]);
        xb_tab[ib + 1] =
        ...
        yb_tab[ib + 1] =
            ...
            sin(theta_tab[ib + 1]);
        if (ib == -1)
          yb_tab[ib + 1] *= -1.0; /* to correct an issue with the fabs of the
                                     very first point */
      }
      ...
      ya1 = shape_a->r[(angle_a_int + (1 - 2 * flip_a) * scan_sign * -1 +
            ...
            sin(theta_ia0);
      ya1 *=
          -1.0; /* to correct an issue with the fabs of the very first point */
      for (ia = -1; ia < SHAPE_RESOLUTION / 2; ia++) {
        theta_ia0 = theta_ia1;
        theta_ia1 =
            fabs((angle_a_int + (1 - 2 * flip_a) * scan_sign * (ia + 1)) * 2.0 *
                     M_PI / SHAPE_RESOLUTION -
                 angle_a);
        xa0 = xa1;
        ya0 = ya1;
        ...
        for (ib = -1; ib < SHAPE_RESOLUTION / 2; ib++) {
          if (segments_cross(xa0, ya0, xa1, ya1, xb_tab[ib + 1], yb_tab[ib + 1],
      ...
      for (ia = -1; ia < (SHAPE_RESOLUTION / 4) + 1; ia++) {
        ...
        for (ib = -1; ib < (SHAPE_RESOLUTION / 4) + 1; ib++) {
          if (segments_cross(xa0, ya0, xa1, ya1, xb_tab[ib + 1], yb_tab[ib + 1],
            ...
            // xa0, ya0, xa1, ya1, xb_tab[ib+1], yb_tab[ib+1], xb_tab[ib+2],
            // yb_tab[ib+2]);
...
bool pair_clash(struct shapetype *shape_a, struct shapetype *shape_b,
                double coords_a[DIM], double coords_b[DIM], double orient_a,
                double orient_b, int rotation_a, int rotation_b, bool flip_a,
                bool flip_b, int tests) {
  double central_distsq, maxshapecontact_distsq;
  double a_to_b_incline, b_to_a_incline;

  double mathtol = 1e-8;

  central_distsq = (coords_a[0] - coords_b[0]) * (coords_a[0] - coords_b[0]) +
                   (coords_a[1] - coords_b[1]) * (coords_a[1] - coords_b[1]);
  ...
  if (central_distsq > maxshapecontact_distsq) {
    /* if they're further apart than even the maximum shape radii measures, no
     * clash*/
    return (0);
  }
  ...
  if (isnan(a_to_b_incline)) {
    ...
    if (fabs((coords_b[0] - coords_a[0]) / sqrt(central_distsq) - 1.0) <
        mathtol) {
      a_to_b_incline = 0.0;
    } else if (fabs((coords_b[0] - coords_a[0]) / sqrt(central_distsq) + 1.0) <
               mathtol) {
      a_to_b_incline = M_PI;
    }
  }
  ...
  a_to_b_incline +=
      (1 - 2 * flip_a) * rotation_a * 2.0 * M_PI / SHAPE_RESOLUTION;
  b_to_a_incline +=
      (1 - 2 * flip_b) * rotation_b * 2.0 * M_PI / SHAPE_RESOLUTION;

  while (a_to_b_incline < 0.0)
    a_to_b_incline += 2.0 * M_PI;
  while (b_to_a_incline < 0.0)
    b_to_a_incline += 2.0 * M_PI;
  while (a_to_b_incline >= 2.0 * M_PI)
    a_to_b_incline -= 2.0 * M_PI;
  while (b_to_a_incline >= 2.0 * M_PI)
    b_to_a_incline -= 2.0 * M_PI;

  return indirect_clash(shape_a, shape_b, coords_a, coords_b, a_to_b_incline,
                        b_to_a_incline, flip_a, flip_b, central_distsq, tests);
}

bool clash_polygon(struct shapetype *shape_a, struct shapetype *shape_b,
                   struct wallpaper_group_type group, double *cellsides[DIM],
                   double *cellangles[DIM - 1],
                   int occupiedsites[MAXNUMOCCSITES],
                   double *sitevariables[MAXNUMOCCSITES][3], int rep_a[2],
                   int rep_b[2], bool flips[numoccsites], int tests) {
  ...
  fractcoords(
      group.wyckoffs[occupiedsites[rep_a[0]]].image[rep_a[1]].coord_coeffs,
      sitevariables[rep_a[0]], fcoords_a);
  fractcoords(
      group.wyckoffs[occupiedsites[rep_b[0]]].image[rep_b[1]].coord_coeffs,
      sitevariables[rep_b[0]], fcoords_b);

  orient_a = *sitevariables[rep_a[0]][2];
  orient_b = *sitevariables[rep_b[0]][2];

  /* this means extra rotations due to the symmetries that generated this image
   * from the site basis, done as an integer offset */
  rotation_a =
      group.wyckoffs[occupiedsites[rep_a[0]]].image[rep_a[1]].rotation_offset;
  rotation_b =
      group.wyckoffs[occupiedsites[rep_b[0]]].image[rep_b[1]].rotation_offset;

  flip_a = group.wyckoffs[occupiedsites[rep_a[0]]].image[rep_a[1]].flipped ^
           flips[rep_a[0]];
  flip_b = group.wyckoffs[occupiedsites[rep_b[0]]].image[rep_b[1]].flipped ^
           flips[rep_b[0]];

  /* this method of only looking at the 9 nearest cells fails for those with
   * extreme angles, so I have limited to around 30 [45, as at 25/11/12] degrees
   */
  ...
  for (cellimgx = -2; cellimgx <= 2; cellimgx++) {
    ...
    for (cellimgy = -2; cellimgy <= 2; cellimgy++) {
      ...
      if ((rep_a[0] == rep_b[0]) && (rep_a[1] == rep_b[1]) && (cellimgx == 0) &&
          (cellimgy == 0)) {
      ...
      if (pair_clash(shape_a, shape_b, coords_a, coords_b, orient_a, orient_b,
                     rotation_a, rotation_b, flip_a, flip_b, tests)) {
        return 1;
      }

      // printf("clash ratios %f %f %d %d [%f %f] [%f %f]\n", min_clash_ratio,
      // clash_ratio, cellimgx, cellimgy, fcoords_a[0], fcoords_a[1],
      // img_fcoords_b[0], img_fcoords_b[1]);
    }
  }
...
double packing_fraction(struct shapetype *shape,
                        struct wallpaper_group_type group,
                        double *cellsides[DIM], double *cellangles[DIM - 1],
                        int occupiedsites[MAXNUMOCCSITES],
                        double *sitevariables[MAXNUMOCCSITES][3],
                        bool flips[numoccsites], int tests,
                        int *clash_rejection) {
  int os;
  int countreplicas = 0;
  ...
  *clash_rejection = 0;
  ...
  if (tests != MAXSTEPS + 1) {
    /* we skip the clash detection procedure entirely if tests=MAXSTEPS+1, which
     * is the case when we are reloading the best structures in the cycle and
     * across all cycles, because reloaded structures have already been assessed
     * for clashes */
    ...
    while ((currclash) && (*clash_rejection == 0) && (iter < MAXITER)) {
      ...
      for (r = 0; r < countreplicas; r++) {
        ...
        if (replicaindex[r][1] == 0) {
          ...
          for (s = 0; s < countreplicas; s++) {
            ...
            // doubling up when checking pairs. the first time we take a shape,
            // double_count is 0 so we check it for clashes against every
            // replica. we then add 1 to double_count. we continue (without
            // checking clashes) until we find a replica on a new occupied site,
            // at which point m=0. we then check this replica against all
            // replicas EXCEPT the original replica we checked when double_count
            // was 0, as this pairing has already been checked for clashes.
            if (tests == 0) {
              currclash =
                  clash_polygon(shape, shape, group, cellsides, cellangles,
                                occupiedsites, sitevariables, replicaindex[r],
                                replicaindex[s], flips, tests);
              if (currclash) {
                if (!EMPTY_OUTPUT)
              ...
              /*clashratio =
              clash_spokes(shape,shape,group,cellsides,cellangles,occupiedsites,sitevariables,replicaindex[r],replicaindex[s],flips);
              if (clashratio<1.0-EPS) {
                currclash=1;
                if ((WARNINGS)&&(clashratio<0.4)) {
                  printf("replica %d (%d, %d) and %d (%d, %d) clash ratio = %f
              cell %f %f\n", r, replicaindex[r][0], replicaindex[r][1], s,
              replicaindex[s][0], replicaindex[s][1], clashratio, *cellsides[0],
              *cellsides[1]);
                }
              }
              ...
              currclash =
                  clash_polygon(shape, shape, group, cellsides, cellangles,
                                ...
                                replicaindex[s], flips, tests);
              if (currclash) {
                /* if we have found any clash, and we are not in the initial
                 * phases (tests>0), then there's no need to search through for
                 * any more. exit the two for loops and the while loop
                 * immediately. also, flag there has been a clash so we can
      ...
      /* if there is a clash, we rescale the cell to avoid the clash only if we
       * have just begun, i.e. before the first step. otherwise, i.e. if there
       * is a clash after any step, we don't rescale the cell */
      ...
      *cellsides[0] = MIN(*cellsides[0], 99.0);
      ...
      /* I HAVE NOT YET TAKEN CARE TO ENSURE THE CELL SIDES STAY WITHIN THEIR
         PROSCRIBED RANGE DURING THIS STEP FOR NOW, a VERY QUICK AND DIRTY
         VERSION OF THIS IS IMPLEMENTED IN THE ABOVE - hardcoded 99.0*/
      //} // else Don't compress at all... allow the MC to do that
    }
  } /* else printf("no need to check for clash, just reloading best
       structure\n"); */

  // printf("iter %d area %f maxrad %f cell %f %f sintheta %f packing %f\n",
  // iter, area(shape), shape->maxr, *cellsides[0], *cellsides[1],
  // sin(*cellangles[0]),
  // countreplicas*area(shape)/((*cellsides[0])*(*cellsides[1])*sin(*cellangles[0])));
  /////////////printf("cell %f %f angle %6.2f packing %f\n", *cellsides[0],
  ///*cellsides[1], *cellangles[0]*180.0/M_PI,
  ///countreplicas*area(shape)/((*cellsides[0])*(*cellsides[1])*sin(*cellangles[0])));

  if (isnan(countreplicas * area(shape) /
            ((*cellsides[0]) * (*cellsides[1]) * fabs(sin(*cellangles[0]))))) {
    printf("nan encountered %f %f %f\n", (*cellsides[0]), (*cellsides[1]),
           fabs(sin(*cellangles[0])));
    exit(1);
  }

  return countreplicas * area(shape) /
         ((*cellsides[0]) * (*cellsides[1]) * fabs(sin(*cellangles[0])));
}

int initialize_structure_in_group(
    struct shapetype *shape, struct wallpaper_group_type group,
    int occupiedsites[MAXNUMOCCSITES], double basis[MAXVAR],
    int basis_fixed[MAXVAR], double basis_range[MAXVAR][2],
    double *cellsides[DIM], double *cellangles[DIM - 1],
    double *sitevariables[MAXNUMOCCSITES][3], bool flips[MAXNUMOCCSITES],
    int checkfile, int *isfile) {
  ...
  if (group.hexagonal) {
    if (!EMPTY_OUTPUT)
      printf("hexagonal group\n");
    basis[0] = M_PI / 3.0;
    basis_range[0][0] = M_PI / 3.0;
    basis_range[0][1] = M_PI / 3.0;
    basis_fixed[0] = 1;
    countvar += 1; /* not actually variable, but always part of the basis, so
                      must be included */
    ...
    basis[0] =
        basis_range[0][0] +
        Fluke() * (basis_range[0][1] -
                   basis_range[0][0]); /*oblique. M_PI/2.5 angle is starting
                                          value only, this will be varied */
    basis_fixed[0] = 0;
    countvar += 1;
  }
  ...
  if (group.a_b_equal) {
    ...
    countvar += 2;
    basis[1] = 4.0 * shape->maxr * countreplicas;
    basis[2] = 4.0 * shape->maxr * countreplicas;
        /* NEW 2012-12-10: starting cell edge length */ /* 2018-05-29 sqrt
                                                           removed because some
                                                           wyckoffs don't spread
                                                           out in both
                                                           dimensions */
    basis_range[1][0] = 0.1;
    basis_range[1][1] = basis[1];
  ...
  for (os = 0; os < numoccsites; os++) {
    countreplicas += group.wyckoffs[occupiedsites[os]].multiplicity;
    if (!EMPTY_OUTPUT)
      printf("%c ", group.wyckoffs[occupiedsites[os]].letter);
    sitevariables[os][0] =
        &zero; // initialize, for some sites there are no site variables
    sitevariables[os][1] = &zero;
    sitevariables[os][2] = &zero;
    if (fabs(group.wyckoffs[occupiedsites[os]].image[0].coord_coeffs[0][0]) >
        0.1) {
      /* then x is variable*/
      ...
      // printf("site variable %p\n", sitevariables[os][0]);
      if (!EMPTY_OUTPUT)
        printf("site x variable[%d] %f\n", os, *sitevariables[os][0]);
      countvar++;
    }
    if (fabs(group.wyckoffs[occupiedsites[os]].image[0].coord_coeffs[1][1]) >
        0.1) {
      /* then y is variable*/
      basis[countvar] = Fluke();
      basis_range[countvar][0] = 0.0;
      ...
      if (!EMPTY_OUTPUT)
        printf("site y variable[%d] %f\n", os, *sitevariables[os][1]);
      countvar++;
    }

    /*choose the orientation of the zeroth image of this particle */ /* This
                                                                        could
                                                                        also be
                                                                        done
                                                                        within
    ...
    if (group.wyckoffs[occupiedsites[os]].sitemirrors) {
      if (group.wyckoffs[occupiedsites[os]].image[0].sitemirror0) {
        ...
        basis[countvar] =
            3.0 * M_PI / 4.0; /*not sure why I need this and the next one the
                                 wrong way around... but it works */
      ...
      basis_fixed[countvar] =
          2; /*this is a different kind of fixed ... we can rotate by certain
                angles, as long as we match the sitemirrors */
      ...
      basis_fixed[countvar] =
          0; /* TO DO: new type of fixing - periodic in 2Pi */
      sitevariables[os][2] = &basis[countvar];
      if (!EMPTY_OUTPUT)
  ...
  sprintf(filename, "%s/solution_%s_%s_%s_%.3f.svg", directory, shape->name,
          group.label, oslist, shape_var);

  if (checkfile == 1) {
    /* try to load existing solution */
    if ((fp = fopen(filename, "r"))) {
      if (!EMPTY_OUTPUT)
        printf("try to load existing solution\n");
      *isfile = 1;
      char line[1024]; /* doesn't matter that the really long lines don't fit in
                          one array?*/
      ...
      *isfile = 0;
      if (!EMPTY_OUTPUT)
        printf("No existing solution -  File: %s cannot be loaded.\n",
               filename);
    }
  }
  return (countvar);
...
void uniform_best_packing_in_isopointal_group(
  ...
  for (cycle = 0; cycle < CYCLES; cycle++) {
    ...
    while (phi < 0.0) {
      if ((cycle == 0) || (isfile == 0))
        basis_size = initialize_structure_in_group(
            shape, group, occupiedsites, basis, basis_fixed, basis_range,
            cellsides, cellangles, sitevariables, flips, checkfile, &isfile);
      ...
      phi = packing_fraction(shape, group, cellsides, cellangles, occupiedsites,
                             sitevariables, flips, tests, &clash_rejection);
    }
    if (!EMPTY_OUTPUT)
      printf("initial packing fraction = %f\n", phi);
    ...
    while (tests < MAXSTEPS) {
      ...
      // if (tests%100==0) vary_index=0; /* only occasionally allow the cell
      // angle to change */
      ...
      if (basis_fixed[vary_index] == 0) {
        if (basis_range[vary_index][1] < 3.0) {
          ...
          basis[vary_index] = old_value + max_step_size *
                                              (basis_range[vary_index][1] -
                                               basis_range[vary_index][0]) *
                                              (Fluke() - 0.5);
          // printf("A kT = %g, phi=%f %d %f, (%f, %f) theta=%f\n", kT, phi,
          // vary_index, old_value, basis_range[vary_index][0],
          // basis_range[vary_index][1], *cellangles[0]);
        } else {
          /* This is a cell side.  Since these have a direct effect on the
           * packing fraction, make their step size in proportion to kT */
          basis[vary_index] =
              old_value * (1.0 + MIN(3.0 * kT, 0.1) * (Fluke() - 0.5));
        }
      ...
      if (!EMPTY_OUTPUT) {
        if (tests % 500 == 0)
          printf("cycle %d of %d, step %ld of %d, kT=%g, packing %f, angle %f, "
                 "b/a=%f, rejection %f percent\n",
                 cycle + 1, CYCLES, tests, MAXSTEPS, kT, phi,
                 *cellangles[0] * 180.0 / M_PI,
                 (*cellsides[1]) / (*cellsides[0]),
                 (100.0 * rejections) / tests);
      }
    }

    tests = MAXSTEPS + 1;
    /* add 1 to tests so that tests=MAXSTEPS+1, to signify to packing_fraction
     * function not to check for clashes when reloading best structures */
    ...
    phi = packing_fraction(shape, group, cellsides, cellangles, occupiedsites,
                           sitevariables, flips, tests, &clash_rejection);
    if (!EMPTY_OUTPUT) {
      printf("BEST: cell %f %f angle %6.2f packing %f rejection (%f\%%) ",
             *cellsides[0], *cellsides[1], *cellangles[0] * 180.0 / M_PI,
             max_phi_in_cycle, (100.0 * rejections) / tests);
      for (i = 0; i < basis_size; i++) {
        printf("%f ", basis[i]);
      }
      for (i = 0; i < numoccsites; i++) {
...
#define btoa(x) ((x) ? "true" : "false")
    ...
    int totalsum = 0;
    for (os = 0; os < numoccsites; os++) {
      chiralsum += (best_flips[os] * 2 - 1) *
                   group.wyckoffs[occupiedsites[os]].multiplicity;
      totalsum += abs((best_flips[os] * 2 - 1) *
                      group.wyckoffs[occupiedsites[os]].multiplicity);
    }
    if (chiralsum != 0) {
      chiral_state = 'c';
    ...
    fprintf(fpout,
            ...
            ".4f\t%.4f\t%.6f\t%.6f\t%.6f\t%.6f\t%c",
            group.label, oslist, cycle + 1, max_phi, area(shape),
            (100.0 * rejections) / tests, shape->name, MAXSTEPS, max_step_size,
            SHAPE_RESOLUTION, shape_var, numoccsites, shape->minr, shape->maxr,
            shape->minr / shape->maxr, max_phi_in_cycle, *cellsides[0],
            *cellsides[1], *cellangles[0] * 180.0 / M_PI, chiral_state);

    for (os = 0; os < numoccsites; os++) {
      fprintf(fpout, "\t%s\t%d", btoa(best_flips[os]),
              group.wyckoffs[occupiedsites[os]].multiplicity);
    }
    fprintf(fpout, "\n");
    fprintf(fp_isopointal_out,
            "%.3f\t%.6f\t%.7f\t%.3f\t%s\t%8s\t%s\t%d\t%d\t%.5f\t%d\t%d\t%.4f\t%"
            ".4f\t%.4f\t%.6f\t%.6f\t%.6f\t%.6f\n",
            shape_var, max_phi, area(shape), (100.0 * rejections) / tests,
            shape->name, group.label, oslist, cycle + 1, MAXSTEPS,
            max_step_size, SHAPE_RESOLUTION, numoccsites, shape->minr,
            shape->maxr, shape->minr / shape->maxr, max_phi_in_cycle,
            *cellsides[0], *cellsides[1], *cellangles[0] * 180.0 / M_PI);
    ...
    shape_scale =
        1.0; /* (image_scale-1.0)/image_scale;  this was originally an attempt
    ...
    origin_translate_x = GRAPHICFRAME + MAX(0.0, -image_scale * *cellsides[1] *
                                                     cos(*cellangles[0]));
    ...
    if (POLYGON_REP == 1) {
      ...
      for (i = 0; i < SHAPE_RESOLUTION; i++) {
        fprintf(fp, "%f,%f ",
                (image_scale * shape->r[i] - 0.5) *
                    cos(i * 2.0 * M_PI / SHAPE_RESOLUTION),
                (image_scale * shape->r[i] - 0.5) *
                    sin(i * 2.0 * M_PI / SHAPE_RESOLUTION));
      }
    ...
    if (AXES_SHOWN) {
      fprintf(fp, "<line x1=\"0\" y1=\"0\" x2=\"20\" y2=\"0\"  "
                  "stroke=\"black\" stroke-width=\"1\"  />\n");
      fprintf(fp, "<line x1=\"0\" y1=\"0\" x2=\"0\" y2=\"20\"  stroke=\"red\" "
                  "stroke-width=\"1\"  />\n");
    }
    if (CENTRE_SPOT) {
      fprintf(fp, "<circle cx=\"0.0\" cy=\"0.0\" r=\"3.0\" fill=\"black\"  "
    ...
    for (os = 0; os < numoccsites; os++) {
      for (m = 0; m < group.wyckoffs[occupiedsites[os]].multiplicity; m++) {
        fractcoords(group.wyckoffs[occupiedsites[os]].image[m].coord_coeffs,
                    sitevariables[os], fcoords);
        for (cell_img_x = 0; cell_img_x < 3; cell_img_x++) {
          for (cell_img_y = 0; cell_img_y < 3; cell_img_y++) {
            fcoords[0] += 1.0 * cell_img_x;
            fcoords[1] += 1.0 * cell_img_y;
            ...
            fprintf(
                ...
                origin_translate_x + image_scale * coords[0],
                origin_translate_y + image_scale * coords[1],
                -((group.wyckoffs[occupiedsites[os]].image[m].flipped ^
                   flips[os]) *
                      (-2) +
                  1) * (*sitevariables[os][2] * 180.0 / M_PI) -
                    group.wyckoffs[occupiedsites[os]].image[m].rotation_offset *
                        360.0 / SHAPE_RESOLUTION,
                shape_scale,
                ((group.wyckoffs[occupiedsites[os]].image[m].flipped ^
                  flips[os]) *
                     (-2) +
                 1) *
                    shape_scale);
            if (cell_img_x == 1 && cell_img_y == 1) {
              fprintf(fp, "rgb(%d,%d,%d)", (int)(0.1 * 256 / (os + 1)),
                      (int)(0.3 * 256 / (os + 1)), (int)(0.6 * 256 / (os + 1)));
            } else {
              fprintf(fp, "rgb(%d,%d,%d)", (int)(0.1 * 256 / (os + 1)),
                      (int)(0.6 * 256 / (os + 1)), (int)(0.3 * 256 / (os + 1)));
            }
            fprintf(fp, "\"/>\n");
          }
        }
    ...
    fprintf(fp,
            ...
            *cellsides[0], *cellsides[1], *cellangles[0] * 180.0 / M_PI, phi,
            max_phi, (100.0 * rejections) / tests);
    fprintf(fp, "-->\n");
    fclose(fp);
  }
}

void uniform_best_packing_in_group(struct shapetype *shape,
                                   struct wallpaper_group_type group) {
  ...
  int combination_bag[MAXNUMOCCSITES *
                      group.num_wyckoffs]; /* temporary variable only */
  int curr_bag = 0;
  ...
  for (wk = 0; wk < group.num_wyckoffs; wk++) {
    if ((shape->rotsym % group.wyckoffs[wk].siterotations == 0) &&
        ((group.wyckoffs[wk].sitemirrors == 0) ||
         ((shape->mirrors % group.wyckoffs[wk].sitemirrors == 0) &&
          (shape->mirrors != 0)))) {
  ...
  /* Count up how many were valid in total,
     and amongst those, how many had some variability (which means they can be
     mulitply occupied). */
  ...
  maxcombinations =
      factorial(n * a + v) / (factorial(n) * factorial(n * a + v - n));
  ...
  printf("estimate: there should be less than %d ways of filling %d sites with "
         "choices from %d variable wyckoffs and %d non-variable wyckoffs \n",
         maxcombinations, n, a, v);

  int occupiedsites[maxcombinations][MAXNUMOCCSITES];

  numcombinations = enumerate(occupiedsites, maxcombinations, 0, 0, 0,
                              combination_bag, curr_bag);
  ...
  for (i = 0; i < numcombinations; i++) {
    if (!EMPTY_OUTPUT) {
      ...
      for (os = 0; os < numoccsites; os++)
        printf(" %c", group.wyckoffs[occupiedsites[i][os]].letter);
      printf("\n");
    }

    char format[] = "%s/output_%s_%s_%c_%d.dat"; // DAN this paragraph
    char filename[sizeof format + 200];
    sprintf(filename, format, directory, shape->name, group.label,
            group.wyckoffs[occupiedsites[i][0]].letter, numoccsites);
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[2], "fourier") == 0) {
    if (argc != 22) {
      fprintf(
          stderr,
          "wrong number of arguments. wallpaper.exe num_occ_sites shape_name "
          "wallpaper_group_num/all/best5/best8 coefficients(a1->a9,b1->b9)\n");
      exit(1);
    }
  } else {
    if (argc != 4) {
      fprintf(stderr, "wrong number of arguments. wallpaper.exe num_occ_sites "
                      "shape_name wallpaper_group_num/all/best5/best8\n");
      exit(1);
    }
  }
  ...
  if (strcmp(argv[2], "fourier") == 0) {
    ...
    sprintf(command,
            "mkdir "
            "%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_"
            "%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f",
            fcoeff[0][0], fcoeff[0][1], fcoeff[0][2], fcoeff[0][3],
            fcoeff[0][4], fcoeff[0][5], fcoeff[0][6], fcoeff[0][7],
            fcoeff[0][8], fcoeff[1][0], fcoeff[1][1], fcoeff[1][2],
            fcoeff[1][3], fcoeff[1][4], fcoeff[1][5], fcoeff[1][6],
            fcoeff[1][7], fcoeff[1][8]);
    sprintf(directory,
            "%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_"
            "%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f_%4.2f",
            fcoeff[0][0], fcoeff[0][1], fcoeff[0][2], fcoeff[0][3],
    ...
    define_RichardJames_pentagon(&RJpentagon,
                                 1.4); /* a bit of an arbitrary one */
    chosen_shape = &RJpentagon;
  } else if (strcmp(argv[2], "RJ2pentagon") == 0) {
    define_RichardJames_pentagon(&RJ2pentagon,
                                 1.2); /* a bit of an arbitrary one */
    chosen_shape = &RJ2pentagon;
  } else if (strcmp(argv[2], "MRpentagon") == 0) {
    define_MarjorieRice_pentagon(&MRpentagon,
                                 1.9); /* angle must be greater than pi/2 */
    chosen_shape = &MRpentagon;
  } else {
    fprintf(stderr, "shape not recognized\n");
    exit(1);
  }
  sprintf(datfilename, "%s/%s_%d_%.3f.dat", directory, argv[2],
          SHAPE_RESOLUTION, shape_var);
  ...
  */
  ...
  char filename[sizeof format + 200];
  sprintf(filename, format, directory, chosen_shape->name, shape_var,
          numoccsites);

  fpout = fopen(filename, "a");

  if (do_all == 1) { // all
    for (study_wallpaper_group = 0; study_wallpaper_group < 17;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
      uniform_best_packing_in_group(chosen_shape,
                                    wallpapergroups[study_wallpaper_group]);
    }
  } else if (do_all == 2) { // DAN: best5 - p1, p2, pg, p2mg, p2gg
    for (study_wallpaper_group = 0; study_wallpaper_group < 2;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 3; study_wallpaper_group < 4;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 6; study_wallpaper_group < 8;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 3; study_wallpaper_group < 5;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 12; study_wallpaper_group < 13;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 14; study_wallpaper_group < 15;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 9; study_wallpaper_group < 10;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 15; study_wallpaper_group < 16;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
  ...
  } else if (do_all == 5) { // DAN: worst9 - pm, p2mm, c2mm, p4, p4mm, p4gm,
                            // p3m1, p6, p6mm
    for (study_wallpaper_group = 2; study_wallpaper_group < 3;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 5; study_wallpaper_group < 6;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 8; study_wallpaper_group < 12;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 13; study_wallpaper_group < 14;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 15; study_wallpaper_group < 17;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
  ...
  } else if (do_all == 6) { // DAN: chiral10 - p1, p2, pg, cm, p2mg, p2gg, p4,
                            // p3, p31m, p6
    for (study_wallpaper_group = 0; study_wallpaper_group < 2;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
    ...
    for (study_wallpaper_group = 14; study_wallpaper_group < 16;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
      uniform_best_packing_in_group(chosen_shape,
             ...
             7) { // DAN: worst7 - pm, p2mm, c2mm, p4mm, p4gm, p3m1, p6mm
    for (study_wallpaper_group = 2; study_wallpaper_group < 3;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
    ...
    for (study_wallpaper_group = 8; study_wallpaper_group < 9;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 10; study_wallpaper_group < 12;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 16; study_wallpaper_group < 17;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
    ...
    for (study_wallpaper_group = 17; study_wallpaper_group < 18;
         study_wallpaper_group++) {
      printf("find the best packing of %s in group %s with %d sites occupied\n",
             chosen_shape->name, wallpapergroups[study_wallpaper_group].label,
             numoccsites);
